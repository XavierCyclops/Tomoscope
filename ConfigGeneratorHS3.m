...
if Generador.Signal == 5
 ...
 if (Opcion == 1) 
     ye     = pulsoUTsin(Generador.Fsig,FpG,Nven);  %Señal Pulso Gaussiano
 end 
 if (Opcion == 2 )
     ye     = pulsoUTcuad(Generador.Fsig,FpG,Nven); %Señal Pulso Cuadrado
 end 
  if (Opcion == 3 )
     ye     = pulsoUTcuadn(Generador.Fsig,FpG,Nven); %Señal Pulso Exponencial
  end 
  
 if (Opcion == 4)
     ye     = pulsoUTsinc(Generador.Fsig,FpG,Nven); %Señal Pulso Sinc
 end 
